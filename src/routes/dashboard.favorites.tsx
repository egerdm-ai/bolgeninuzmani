      ...
      <section className="space-y-3">
        ...
        {saved.length > 0 ? (
          ...
          <EmptyStateCard
            ...
            action={<Button asChild className="bg-gradient-gold text-primary-foreground hover:opacity-90"><Link to="/dashboard/search">Portföy Ara</Link></Button>}
          />
        )}
      </section>
          ...
          {savedSearches.map((s) => (
            <SurfaceCard key={s.id} hover className="space-y-2">
              <div className="flex items-start justify-between">
                ...
                <span className="flex size-8 items-center justify-center rounded-lg bg-gold/10 text-gold"><Search className="size-4" /></span>
              </div>
              <p className="text-xs text-muted-foreground">"{s.query}"</p>
              <div className="flex flex-wrap gap-1.5">
                {s.filters.map((f) => (
                  <span key={f} className="rounded-md bg-surface-3 px-2 py-0.5 text-xs text-secondary-foreground">{f}</span>
                ))}
              </div>
              <div className="flex items-center justify-between pt-1">
                <span className="text-xs text-gold">{s.resultCount} sonuç</span>
                <Button asChild variant="ghost" size="sm" className="text-gold hover:text-gold-light"><Link to="/dashboard/search">Aramayı Aç</Link></Button>
              </div>
            </SurfaceCard>
          ))}
        </div>

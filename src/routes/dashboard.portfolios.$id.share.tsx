...
function ShareStudio() {
  ...
  const analyticsRows = [
    ...
    { icon: MessageCircle, label: "WhatsApp tıklaması", value: formatNumber(analytics.whatsappClicks) },
  ...
  return (
    <PageContainer className="space-y-6">
      ...
      <div className="flex items-start gap-3 rounded-2xl border border-gold/30 bg-gold/[0.06] px-4 py-3">
        ...
        <div>
          <p className="text-sm font-semibold text-foreground">Kilitli bilgiler yalnızca detay talebi sonrası açılır</p>
          <p className="text-xs text-muted-foreground">
            Tam adres, tapu ve PDF belgeleri paylaşımda gizli kalır; yalnızca onayladığınız detay taleplerinde görünür hale gelir.
          </p>
        </div>
      </div>
          ...
          {channel === "whatsapp" && (
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <SurfaceCard className="space-y-3">
                ...
                <div className="rounded-2xl bg-[#0b141a] p-4">
                  <div className="ml-auto max-w-[92%] rounded-2xl rounded-tr-sm bg-[#005c4b] p-3 shadow">
                    ...
                    <p className="mt-2 whitespace-pre-line text-[12px] leading-relaxed text-[#e9edef]">{message}</p>
              ...
              <SurfaceCard className="space-y-4">
                ...
                <Textarea value={message} readOnly rows={11} className="resize-none font-mono text-xs" />
            ...
            <SurfaceCard className="space-y-4">
              ...
              <div className="rounded-xl border border-border bg-surface-2 p-3">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Bu link her açılışta tıklanma, ziyaretçi ve detay talebi olarak ölçülür. (Demo — yakında)
                </p>
                <div className="mt-2 flex flex-wrap gap-2">
                  <span className="rounded-md bg-surface-3 px-2 py-0.5 text-[11px] text-secondary-foreground">UTM destekli</span>
                  <span className="rounded-md bg-surface-3 px-2 py-0.5 text-[11px] text-secondary-foreground">Cihaz kırılımı</span>
                  <span className="rounded-md bg-surface-3 px-2 py-0.5 text-[11px] text-secondary-foreground">Coğrafi dağılım</span>
                </div>
              </div>
            ...
            <SurfaceCard className="space-y-4">
              ...
              <div className="relative aspect-[3/4] max-w-xs overflow-hidden rounded-xl border border-border">
                ...
                <div className="absolute bottom-0 p-4">
                  <p className="line-clamp-2 text-sm font-semibold text-foreground">{portfolio.title}</p>
              ...
              <div className="flex gap-2">
                <Button onClick={() => toast.success("PDF önizleme açıldı (demo)")} variant="outline" className="flex-1 gap-1.5">
                  <Eye className="size-4" /> Önizle
                </Button>
                <Button onClick={() => toast.success("PDF indiriliyor (demo)")} className="flex-1 gap-1.5 bg-gradient-gold text-primary-foreground hover:opacity-90">
        ...
        <div className="space-y-5 lg:sticky lg:top-20 lg:self-start">
          ...
          <SurfaceCard className="p-0">
            <div className="flex items-center justify-between border-b border-border px-4 py-3">
              ...
              <span className="rounded-full bg-surface-2 px-2 py-0.5 text-[10px] uppercase tracking-wide text-muted-foreground">Demo</span>
            </div>
            <div className="grid grid-cols-2 gap-px bg-border">
              {analyticsRows.map((r) => (
                <div key={r.label} className="bg-surface p-4">
                  <r.icon className="size-4 text-gold" />
                  <div className="mt-2 font-display text-xl font-semibold text-foreground">{r.value}</div>
                  <div className="text-[11px] text-muted-foreground">{r.label}</div>
                </div>
              ))}
            </div>
            <p className="px-4 py-2 text-[11px] text-muted-foreground">Gerçek zamanlı paylaşım istatistikleri yakında aktif olacak.</p>
          </SurfaceCard>
        </div>
      </div>
    </PageContainer>

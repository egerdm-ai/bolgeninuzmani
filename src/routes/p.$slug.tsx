...
export const Route = createFileRoute("/p/$slug")({
  ...
  head: ({ loaderData }) => ({
    meta: [
      { title: `${loaderData?.portfolio.title ?? "Portföy"} — Bölgenin Uzmanı` },
      { name: "description", content: loaderData?.portfolio.shortDescription ?? "" },
      { property: "og:title", content: loaderData?.portfolio.title ?? "Bölgenin Uzmanı Portföy" },
...
function PublicPortfolioPage() {
  ...
  const similar = portfolios
    .filter(
      (p) => p.id !== portfolio.id && p.category === portfolio.category && p.status === "active",
    )
    .slice(0, 3);
          ...
          <div className="flex items-center gap-2">
            <Button asChild variant="ghost" size="sm" className="gap-1.5 text-muted-foreground">
              <Link to="/dashboard/search">
                <ArrowLeft className="size-4" /> Aramaya Dön
              </Link>
            </Button>

...
| B9 | Bölgeler | ✅ uçtan uca bağlı | `20260620170000_b9_region_summary` (UYGULANDI) | RPC push + tipler regen. `lib/data/regions.ts` gerçek `supabase.rpc('get_region_summary')`. `dashboard.regions.index` (ilçe + aktif sayı, sıralı) → `regions.$slug` (o bölgenin portföyleri + PortfolioMap approx pin) + "Keşfet'te Filtrele" (`/search?q=district`). `featureFlags.regions=true` + sidebar "Bölgeler". Leak'e eklendi. |
